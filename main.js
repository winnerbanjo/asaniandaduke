document.addEventListener('DOMContentLoaded', () => {
  ...
  forms.forEach(form => {
    form.addEventListener('submit', (e) => {
      ...
      if (isValid) {
        // Check if newsletter, volunteer, festival, or contact form
        if (form.classList.contains('newsletter-form')) {
          ...
          form.reset();
        } else if (form.id === 'festival-reg-form') {
          ...
          const regData = {
            ...
            org: document.getElementById('reg-org') ? document.getElementById('reg-org').value : '',
            message: document.getElementById('reg-message') ? document.getElementById('reg-message').value : '',
            date: new Date().toISOString()
          };
          
          try {
            const existingRegs = JSON.parse(localStorage.getItem('festival_registrations') || '[]');
            existingRegs.push(regData);
            localStorage.setItem('festival_registrations', JSON.stringify(existingRegs));
          } catch (err) {
            console.error("Local storage save failed:", err);
          }
          
          showToast("Registration successful! See you at the Festival.");
          form.reset();
